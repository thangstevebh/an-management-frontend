...
import { addWithdrawCommandSchema } from "@/lib/validation/add-withdraw-command-schema";
import {
...
export default function Page() {
  ...
  const addWithdrawCommandMutation = useMutation({
    mutationKey: ["add-withdraw-command", wasSubmitted, cardId],
    mutationFn: async (data: {
      amount: number;
      cardId: string;
      note?: string;
    }) => {
      const { cardId, amount: withdrawRequestedAmount, ...restPayload } = data;

      const response = await useAxios.post(
        `agent/add-withdraw-command/${cardId}`,
        {
          withdrawRequestedAmount: withdrawRequestedAmount,
          ...restPayload,
        },
        {
          headers: {
    ...
    onError: (error) => {
      toast.error("Tạo lệnh rút tiền thất bại", {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
     ...
     * */
    const validationResult = addWithdrawCommandSchema.safeParse(checkData);
    ...
    addWithdrawCommandMutation.mutateAsync({
      ...createPayload,
    });
  };
  ...
  return (
    <div className="">
      ...
      <div className="flex flex-col items-center justify-center w-full gap-4 mb-4">
        <h1 className="text-2xl font-bold mb-4">Thêm lệnh rút tiền vào thẻ</h1>
        <p className="text-center mb-4 text-gray-500">
          Bạn có thể tạo lệnh rút tiền vào thẻ để quản lý giao dịch. Vui lòng
          nhập thông tin lệnh rút tiền và nhấn "Tạo" để thêm vào danh sách lệnh
          nạp tiền.
        </p>
      </div>
          ...
          >
            ...
            <div className="flex flex-col items-start justify-between gap-2">
              ...
              <div className="relative w-full">
                <ValidatedInput
                  ...
                  wasSubmitted={wasSubmitted}
                  fieldSchema={addWithdrawCommandSchema.shape["note"]}
                />
              </div>
            </div>
            ...
            <div className="flex items-center justify-center space-x-4 mt-4">
              <Button
                ...
                disabled={addWithdrawCommandMutation.isPending}
              >
                {addWithdrawCommandMutation.isPending ? (
                  <>
                    Đang tạo{" "}
